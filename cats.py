...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    select_status = [paragraphs[i] for i in range(0, len(paragraphs)) if select(paragraphs[i])]
    if (k<len(select_status)):
        return select_status[k]
    return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    def selector(paragraph):
        pa = remove_punctuation(paragraph)
        par = lower(pa)
        parawords = split(par)
...
def accuracy(typed, source):
    ...
    if (len(typed)<len(source)):
            updated_source_words = source_words[0: len(typed_words)]
            x = len(typed_words)-1
            while x>=0:
                if typed_words[x] == updated_source_words[x]:
                    count += 1.0
                x = x-1
            return (count/len(typed_words)) * 100
    if (len(typed)>len(source)):
        updated_typed_words = typed_words[0: len(source_words)]
        x = len(source_words)-1
        while x>=0:
            if updated_typed_words[x] == source_words[x]:
                count += 1.0
            x = x-1
        return (count/len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    current_word = typed_word
    difference = limit + 1
    for i in word_list:
        if typed_word == i:
            return typed_word
        else:
            if diff_function(typed_word, i, limit) < difference and diff_function(typed_word, i, limit) <= limit:
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(0, len(typed)):
        ...
        if typed[i] != prompt[i]:
            break
    upload({'id': user_id, 'progress': counter/len(prompt)*1.0})
    return counter/len(prompt) * 1.0
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    times = []
    x = 0
    y = len(times_per_player)
    while x < y:
        newtimes = []
        z = 0
        while z < len(times_per_player[x]) - 1:
            newtimes.append(abs(times_per_player[x][z+1] - times_per_player[x][z]))
            z = z+1
        times.append(newtimes)
        x = x+1
...
def fastest_words(match):
    ...
    for y in word_indices:
        ...
        newlist[player].append(get_word(match, y))
    return newlist
